...
function plv_plugin_setting_embedcode() {
    $options = get_option( 'plausible_viewer_options' );
    echo "<div><textarea id='plv_plugin_setting_embedcode_textarea' name='plausible_viewer_options[embedcode]' class='codeeditor' style='width: 80%;' rows='3' />". esc_attr( $options['embedcode'] ) ."</textarea></div><small> Enter the embed Code to enable tracking (optional)<br>Tracking is disabled to editors and administrators. Open your website in a private window to test tracking!</small>";
}



function my_plugin_settings_link($links) { 
  $settings_link = '<a href="options-general.php?page=plausible_viewer">'.esc_html__( 'Settings', 'wp' ).'</a>'; 
